...
const UserAnalytics = ({ isDashboard }: Props) => {
  ...
  const analyticsData: UserItem[] = [];

  if (data?.users?.last12Months) {
    data.users.last12Months.forEach((item: UserAnalyticsMonth) => {
      ...
      {isLoading ? (
        ...
        <div
          className={`${
            isDashboard
              ? "mt-[50px] ml-[50px]"
              : "mt-[50px] ml-[50px] dark:bg-[#111C43] shadow-sm pb-5 rounded-sm "
          }`}
        >
